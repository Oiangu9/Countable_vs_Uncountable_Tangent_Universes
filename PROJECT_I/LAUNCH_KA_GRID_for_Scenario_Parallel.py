...
if __name__ == "__main__":
    ...
    - path to the .py file defining the psi0(x,y) and potential(x,y) functions (dont include .py)
    ...
    if not f"Reference_SE_Simulation" in done:
        ...
        arguments_for_workers.append(("Reference_SE_Simulation", f"./SIMULATORS/SIMULATOR_{dim}D_CN_Schrodinger_Equation.py",
    ...
    for K in Ks_to_try:
        for A in As_to_try:
            ...
            if ID not in done:
                to_do.append(ID)
                arguments_for_workers.append((ID, f"./SIMULATORS/SIMULATOR_MD_Interacting_Newtonian_Universes.py",
